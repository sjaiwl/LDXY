...
public class Personal_shoucang extends Activity implements IXListViewListener,
	...
	private ImageView shoucang_blank;
	...
	private void initView() {
		mylistview = (XListView) this.findViewById(R.id.shoucang_listview);
		shoucang_blank=(ImageView) this.findViewById(R.id.shoucang_blank);
	}
		...
		if (!UserInfo.isEmpty()) {
			...
			mylistview.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> adapter, View view,
					...
					intent.putExtra("activityid", activitylist.get(position - 1).getActivity_id());
					intent.setClass(Personal_shoucang.this, Detailpage.class);
			...
			shoucang_blank.setVisibility(View.VISIBLE);
		}
	}

	private void getData(final int method) { // type=1 重新生成list type=2 增长list
		String url = Configuration.getdynamicshoucangUrl + "?index=" + index+ "&&user_id=" + user_id;
		JsonArrayRequest jar = new JsonArrayRequest(url,
				new Response.Listener<JSONArray>() {
					@Override
					public void onResponse(JSONArray response) {
						List<ActivityUnit> list = JSON.parseArray(response.toString(), ActivityUnit.class);
						if (method == 1) {
							activitylist.clear();
						}
						...
						if (!activitylist.isEmpty()) {
							lastActivityId = activitylist.get(activitylist.size() - 1).getActivity_id();
